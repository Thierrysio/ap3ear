 ...
 */
final class Version20251029215324 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TABLE game4_duel (id INT AUTO_INCREMENT NOT NULL, game_id INT NOT NULL, player_a_id INT NOT NULL, player_b_id INT NOT NULL, winner_id INT DEFAULT NULL, status VARCHAR(16) NOT NULL, created_at DATETIME NOT NULL COMMENT \'(DC2Type:datetime_immutable)\', resolved_at DATETIME DEFAULT NULL COMMENT \'(DC2Type:datetime_immutable)\', logs LONGTEXT DEFAULT NULL, INDEX IDX_9691516FE48FD905 (game_id), INDEX IDX_9691516F99C4036B (player_a_id), INDEX IDX_9691516F8B71AC85 (player_b_id), INDEX IDX_9691516F5DFCD4B8 (winner_id), INDEX idx_g4duel_game_status (game_id, status), INDEX idx_g4duel_game_playerA (game_id, player_a_id), INDEX idx_g4duel_game_playerB (game_id, player_b_id), UNIQUE INDEX uniq_g4duel_pair_pending (game_id, player_a_id, player_b_id, status), UNIQUE INDEX uniq_duel_playerA_pending (game_id, player_a_id, status), UNIQUE INDEX uniq_duel_playerB_pending (game_id, player_b_id, status), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB');
        $this->addSql('CREATE TABLE game4_duel_play (id INT AUTO_INCREMENT NOT NULL, duel_id INT NOT NULL, player_id INT NOT NULL, card_id INT DEFAULT NULL, card_code VARCHAR(64) NOT NULL, card_type VARCHAR(16) NOT NULL, round_index INT NOT NULL, submitted_at DATETIME NOT NULL COMMENT \'(DC2Type:datetime_immutable)\', INDEX IDX_B0B5D9A199E6F5DF (player_id), INDEX IDX_B0B5D9A14ACC9A20 (card_id), INDEX idx_duel (duel_id), INDEX idx_duel_player (duel_id, player_id), INDEX idx_duel_round (duel_id, round_index), UNIQUE INDEX uniq_duel_player_round (duel_id, player_id, round_index), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB');
        ...
        $this->addSql('CREATE TABLE game4_player (id INT AUTO_INCREMENT NOT NULL, game_id INT NOT NULL, incoming_duel_id INT DEFAULT NULL, equipe_id INT NOT NULL, name VARCHAR(100) NOT NULL, role VARCHAR(12) NOT NULL, lives INT DEFAULT 3 NOT NULL, is_alive TINYINT(1) DEFAULT 1 NOT NULL, locked_in_duel TINYINT(1) DEFAULT 0 NOT NULL, INDEX IDX_79695E24E48FD905 (game_id), INDEX IDX_79695E2458994BC (incoming_duel_id), INDEX idx_g4player_game_alive (game_id, is_alive), INDEX idx_g4player_game_role (game_id, role), INDEX idx_g4player_game_locked (game_id, locked_in_duel), UNIQUE INDEX uniq_game_equipe (game_id, equipe_id), PRIMARY KEY(id)) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB');
        ...
        $this->addSql('ALTER TABLE game4_duel ADD CONSTRAINT FK_9691516FE48FD905 FOREIGN KEY (game_id) REFERENCES game4_game (id)');
        $this->addSql('ALTER TABLE game4_duel ADD CONSTRAINT FK_9691516F99C4036B FOREIGN KEY (player_a_id) REFERENCES game4_player (id)');
        $this->addSql('ALTER TABLE game4_duel ADD CONSTRAINT FK_9691516F8B71AC85 FOREIGN KEY (player_b_id) REFERENCES game4_player (id)');
